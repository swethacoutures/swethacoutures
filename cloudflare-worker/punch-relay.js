 ...
 *
 * That first clause is the whole story: whatever we put in `Date`, the terminal believes.
 * It is why the clock "went back to its own time" every time it was set by hand, and why
 * running ZKBioTime appeared to fix it — that is simply another ADMS server sending its
 * own `Date`, and closing it handed the clock back to us.
 *
 * 🔴 THE SHIFT IS ZERO, AND CHANGING IT IS ALMOST CERTAINLY WRONG.
 *
 * This was 480 (−150 minutes of "compensation") on the theory that ZKTeco ships from China
 * on Beijing time. Measured 2026-09-04: the shop's terminal was already on +05:30, so the
 * shift corrected nothing and put the displayed clock 2h30m BEHIND. Staff read the wrong
 * time off the wall for two days. A terminal that keeps correct time must be sent the
 * correct time.
 *
 * A device that genuinely sits on another timezone is now handled where it belongs — in
 * the Vercel function, per device, from measured drift rather than from a guess. See
 * `noticeClockDrift` in api/_deviceIngest.ts. That correction cannot travel through this
 * Worker, because the response below is rebuilt from scratch and drops the origin's
 * headers, so a shop running through Cloudflare gives up automatic clock correction.
 *
 * Which is one more reason this relay is now a fallback, not the main path: the terminal
 * reaches https://swethacoutures.vercel.app directly. See docs/BIOMETRIC_DEVICE.md.
 */
const DATE_SHIFT_MINUTES = 0;
...
function deviceHeaders() {
  return {
    ...
    // The honest clock. Read the block above before ever making this non-zero.
